...
dir_path = "C:/Python Programs/TSF/Task-2/images/"
...
for count, image, file in zip(range(1,len(images)), images, os.listdir(dir_path)):
    img = plt.imread(dir_path+file)
    plt.imshow(img); plt.show()
    time.sleep(1)
    print(get_colors(image=image, no_of_colors=5, show_chart=True))
    time.sleep(1)
    print(f"{count} done, {len(images)-count} more to go!")
